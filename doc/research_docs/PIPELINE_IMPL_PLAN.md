...
| 2026-08-04 | **数据层迁 LeRobot v2.1（grill：互通优先/先迁完再开训/全视频 lossless/手写 reader）**：`data/lerobot/<suite>/` 成训练主数据源（parquet+libx264rgb qp0 无损视频+meta；对齐 starVLA 样例）；agentview_512 approach 段视频统一 rgb0+hindsight（密度 ~8→~40 帧/demo，extract-hindsight 当日废弃）；Stage2LeRobotDataset 与旧版逐元素 parity 全过；benchmark cached 1.46× + RAM 17G→3.5G（纯 stream 较慢但非训练瓶颈）；前提核查纠偏：旧 stream 慢因=整 demo 每 item 重读，非 gzip。踩坑：pthread 耗尽（无界容器缓存×AUTO 线程）与跨 seek 在飞帧（flush_buffers 修） |
