...
### 4.3 Story 骨架：V / L / A 的动静不匹配（2026-08-03，用户提出）

**论点。** VLA 里的三个模态在时间上根本不同类：L（语言指令）是静态的——prompt
给定之后，无论执行得怎样它都不再变化；V 和 A 是动态的——每一个控制步都在更新。
既然如此，让一个几十亿参数的 VLM 在每个控制步都重新"理解一遍语言"，是结构性浪费。
正确的形状是：**L 只被理解一次**，摊销成一个紧凑的静态量（意图向量 / 目标分布 /
judge），此后由轻量的动态回路（vision + flow + action）闭环逼近这个静态目标。

**与本设计的对应。** 这不是新增模块，而是把已有决策统一成一条原则：

- "理解一次"＝ C-VLM 每个 episode 只在初始帧上跑一次，z 整集冻结（rollout 实现即如此）；
- "摊销成静态量"＝ z 与接触分布。注意 z 不是纯语言 embedding，而是**语言×当前场景
  的 grounded 绑定**——这是与"缓存一个 BERT 向量"（BC-Z / ATM 的做法）的本质区别；
- "动态回路不再碰 L"＝ D3：text 从 L3 移除，z 是 flow 的唯一任务通道；
- "L 何时需要重新参与"＝ 相位边界。语言不是每步参与也不是永不参与，而是在原子任务
  切换处重新 ground 一次（libero_10 多原子任务的周期切分 v2 就是它的实现）。
  这是 dual-system 工作普遍没有回答的问题。

**前人坐标。** dual-system VLA（GR00T N1、Figure Helix、Hi Robot、RT-H）已经在用
"慢 VLM + 快策略"的频率分层，理由也正是动静不匹配；LCB 用 latent code 桥接语言与
策略。所以观察本身不孤立，差异点在上面两条：grounded 的静态量、相位边界作为
重新参与的时机。效率论据可以量化：VLA 每个控制步跑 3-7B 参数，我们每个 episode
跑一次 DINO + 7.5M。

**必须正面写的风险与它的反转。** z 是信息瓶颈：z 里没有的任务信息，动态回路无法
恢复。但接口窄恰恰意味着接口质量可测、可归因、可替换——这是相对黑盒 VLA 的
可解释性卖点。

**关键实证（2026-08-03/08-04 两轮测量，含一次撤正）。** 这条原则必须靠**结构强制**
才成立，但强制的对象比想象的更深一层：

- 实测所有已训模型（含最好的冠军）z 对 flow 输出的影响只有 **0.5–0.9%**——z 通道
  形同虚设，这一点成立；
- 初判"旁路 = L4 的 task_emb 文本输入"是**错的**：控制变量复测显示 task_emb 对
  策略输出的影响精确为 0.0（`use_language_token: false` 从阶段〇配置起一直关闭，
  文本从未进入策略）。该接口现已从 approach 分支代码中正式摘除（内部零占位喂
  upstream 管道），并有单测钉住；
- **真正的旁路是图像里的场景布局**：libero_spatial 每个任务的物体摆放不同，策略
  与 L3 只看画面即可在分布内识别任务，梯度因此永远不需要 z。这解释了分布内有分、
  ood 仅 0.1、z 装饰品、上游 L1 改进无法传导的全部现象。
- 推论：视觉不可能从策略中移除，所以断这条旁路只能在**数据层**做——训练集必须包含
  "同一场景、多个目标、仅语言可区分"的任务（libero_goal 即现成工具），迫使梯度
  走 z→flow。z 置换测试在这类任务上应从"无影响"翻转为"毁灭性"，构成现成的
  因果诊断实验（正是我们批评 PPI 只做 ablation 时承诺要补的东西）。
